...
import { View, Text, StyleSheet, TouchableOpacity, Image,SafeAreaView, TextInput, Alert, Modal, FlatList, ActivityIndicator} from 'react-native';
...
export default function Screen ({navigation, route}) {
  ...
  useEffect(() => {
    async function init(){
        ...
        const response = await api.get(`/cameras/camerascidade/${cidade}/${token}`).then(res=>{
          setLoading(false)
          return res
      })
...
const renderItem2 = ({ item }) => (
  <View  style={styles.space}>
    ...
    onPress={() => navigation.navigate('Bairros', {bairro: item.bai_codigo, titulo: item.bai_nome, cidade: cidade})}>
